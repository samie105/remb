...
export async function updateProject(
  id: string,
  input: { name?: string; description?: string | null; status?: string; website_url?: string | null }
): Promise<ProjectRow> {
  const user = await requireUser();
  const db = createAdminClient();

  const updates: { name?: string; description?: string | null; status?: string; slug?: string; website_url?: string | null } = { ...input };
