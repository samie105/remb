...
export async function POST(request: Request) {
  ...
  const { content, projectSlug, type, metadata, sessionId } = body as {
    content?: string;
    projectSlug?: string;
    type?: string;
  ...
  const validTypes = ["summary", "tool_call", "milestone"];
  const entryType = validTypes.includes(type ?? "") ? (type as "summary" | "tool_call" | "milestone") : "summary";
  ...
  return NextResponse.json({ logged: true, id: entry.id, created_at: entry.created_at });
}
